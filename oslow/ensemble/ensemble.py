...
class EnsembleTrainer:
    ...
    def train(self, epoch: int, permutations: torch.Tensor, lbl: str):
        ...
        for batch in self.flow_ensemble_dataloader:
            ...
            log_probs = self.model.log_prob(batch_repeated, perm_mat=perm_mats)
    ...
    def visualize_scatterplot(self):
        for first_side in self.probabilities_evaluation.keys():
            for second_side in self.structure_evaluation.keys():
                ...
                wandb.log({f"scatterplot/{first_side}_vs_{second_side}": wandb.Image(img_data)})
    ...
    def get_log_prob(self, perm: torch.Tensor) -> float:
        ...
        for batch in self.flow_dataloader:
            ...
            log_probs.append(self.model.log_prob(batch, perm_mat=perm_repeated).mean().item())
    ...
    def run(self):
        ...
        for epoch in tqdm(range(self.max_epochs)):
            ...
            if (epoch + 1) % self.logging_scatterplot_frequency == 0:
                ...
                for perm in self.perm_list:
                    self.probabilities_evaluation['ensemble_log_prob'].append(self.get_log_prob(perm))
        ...
        if self.train_fixed:
            for perm in self.perm_list:
                ...
                self.probabilities_evaluation['fixed_log_prob'].append(self.get_log_prob(perm))
