...
class ScoreBasedTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.perform_final_buffer_search and not hasattr(
    ...
    def final_phase(self):
        if not self.perform_final_buffer_search:
            return

        final_phase_buffer_size = len(
            self.permutation_learning_module.permutation_buffer
        )
        final_phase_epoch_count = (
            self.max_epochs
            * (self.permutation_frequency + self.flow_frequency)
            // final_phase_buffer_size
        )
        if (
            final_phase_epoch_count > 0
            and final_phase_buffer_size > 0
            and hasattr(self.permutation_learning_module, "update_buffer")
        ):
            cap = min(10, len(self.permutation_learning_module.permutation_buffer))
            candidate_permutations = (
                self.permutation_learning_module.permutation_buffer[:cap].cpu().numpy()
            )
            best_avg_loss = None
            best_perm = None
            ...
            def tmp_fn(perm_list, best_avg_loss, best_perm, lbl):
                ...
                self.flow_optimizer = self.flow_optimizer_fn(self.model.parameters())
            ...
            _, _ = tmp_fn(best_perm, best_avg_loss, best_perm, lbl="final_flow")
