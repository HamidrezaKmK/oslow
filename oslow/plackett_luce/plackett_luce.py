...
from .multi_task_scheduling import DWAMultiTaskAnnealing
...
class PlackettLuceTrainer:
    def __init__(
        ...
        self.optimizer_instantiate = optimizer
        self.lr_scheduler_instantiate = lr_scheduler
        self.perm_expectation_b_size = perm_expectation_b_size

        self.permutation_log_scores = torch.nn.Parameter(
            torch.zeros(len(dag.nodes), device=device)
        ...
        self.multi_task_scheduler = DWAMultiTaskAnnealing(num_tasks=2, temperature=1.0)

    def run(self):
        self.model.train()
        self.model = self.model.to(self.device)

        self.optimizer = self.optimizer_instantiate(list(self.model.parameters()) + [self.permutation_log_scores])
        self.lr_scheduler = self.lr_scheduler_instantiate(self.optimizer)
        ...
        for epoch in tqdm(range(self.max_epochs)):
            ...
            for batch in self.dataloader:
                ...
                denom = (log_probs.detach().exp()).mean(dim=1) + 1e-6

                flow_loss = -((log_probs * log_probs.detach().exp()).mean(dim=1) / denom).mean()
                perm_loss = -((log_perm_probs * log_probs.detach().exp()).mean(dim=1) / denom).mean()
                ...
                wandb.log({f"step": self.step_count})
                ...
                wandb.log({"perm_coeff": perm_coeff})
                wandb.log({"flow_coeff": flow_coeff})

            # Perform a learning rate scheduling step
            if isinstance(self.lr_scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                self.lr_scheduler.step(sum(avg_loss) / len(avg_loss))
            else:
